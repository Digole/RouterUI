<template>
  <section>
    <div class="line_02"><span>{{$t('staticRouter.title')}}</span></div>
    ...
    <el-table :data="info" @selection-change="selChange" style="width: 100%;" :header-cell-style="headerStyle">
      ...
      <el-table-column :label="$t('operation.operation')" min-width="60">
        <template slot-scope="scope">
          <el-button size="small" @click="deleteRouter(scope.$index, scope.row)"
          v-if="(scope.row.route_type==='-host')||(scope.row.route_type==='-net')"
          >{{$t('operation.delete')}}</el-button>

          <el-button size="small" @click="editRouter(scope.$index, scope.row)"
